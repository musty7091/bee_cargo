...
def record_payment(isletme_id):
    ...
    # Sadece henüz mahsuplaşmamış (isletmeye_aktarildi_mi == False) ve teslim edilmiş kargoları listele
    mahsuplasacak_kargolar_db = Kargolar.query.filter(
        Kargolar.isletme_id == isletme_id,
        Kargolar.isletmeye_aktarildi_mi == False, # ÖNEMLİ FİLTRE
        Kargolar.kargo_durumu == KargoDurumEnum.TESLIM_EDILDI
    ).order_by(Kargolar.olusturulma_tarihi.asc()).all()

    kargolar_data_template = []
    # Kümülatif bakiye hesaplaması artık JS tarafında yapıldığı için Python'da önceden hesaplamaya gerek yok.
    # Ancak, her bir kargonun net etkisini template'e göndermek gerekiyor.
    for kargo_db_item in mahsuplasacak_kargolar_db:
        ...
        kargolar_data_template.append({
            ...
            'bu_satirin_net_etkisi_checkbox': bu_kargonun_kendi_icindeki_net_etkisi # JS'in kullanması için
        })
    ...
    if request.method == 'POST':
        ...
        secilen_kargo_idler_str = request.form.getlist('kargo_ids') # Checkbox'lardan gelen ID'ler
        ...
        if not secilen_kargo_idler_str: # En az bir kargo seçilmeli
        ...
        try:
            ...
            hesaplanan_net_odeme_tutari_islem = Decimal('0.00') # Seçilen kargolara göre hesaplanacak
            ...
            for kargo_id_str_payment in secilen_kargo_idler_str:
                ...
                # Güvenlik kontrolü: Kargo gerçekten bu işletmeye mi ait, durumu uygun mu, daha önce kapatılmamış mı?
                if not (kargo_veritabanindan and kargo_veritabanindan.isletme_id == isletme_id and \
                    ...
                    flash(f"Seçilen kargolardan biri (ID: {kargo_id_str_payment}) ödeme/mahsup için artık uygun değil veya bulunamadı.", "error")
                    return render_template('admin_record_payment.html', isletme=isletme_obj_payment, kargolar_data=kargolar_data_template, today_date=today_date_str)
            ...
            # Yeni ödeme/mahsup kaydı oluştur
            yeni_odeme = IsletmeOdemeleri(
                isletme_id=isletme_id,
                odeme_tarihi=odeme_tarihi,
                odenen_tutar=hesaplanan_net_odeme_tutari_islem, # Hesaplanan net tutar
                aciklama=islem_referansi # Açıklama/Referans
            )
            db.session.add(yeni_odeme)
            db.session.flush() # Yeni ödeme ID'sini almak için
            
            # Seçilen kargoları "aktarıldı" olarak işaretle ve ödeme ile ilişkilendir
            ...
            create_notification(
                ...
                link_endpoint='bp_business.payment_details', # İşletmenin kendi ödeme detayını görebileceği link
            ...
            else: # Negatifse, işletmeden alacak kaydedildi anlamına gelir
        ...
        except ValueError: # Tarih format hatası için
            flash('Geçersiz tarih formatı. Lütfen २०२२-AA-GG formatında girin.', 'error')
        except Exception as e:
            db.session.rollback()
        ...
        # Hata durumunda formu ve seçili kargoları (eğer varsa) tekrar göster
        return render_template('admin_record_payment.html', isletme=isletme_obj_payment, kargolar_data=kargolar_data_template, today_date=today_date_str, secilen_kargo_idler=secilen_kargo_idler_str)
